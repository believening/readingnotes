...
def about(subject):
    ...
    def contain(paragraph):
        parts = split(lower(remove_punctuation(paragraph)))
        for sub in subject:
            for part in parts:
                if part == sub:
                    return True
    ...
    return contain
    # return lambda x: any([
    #     any([part == sub for part in split(lower(remove_punctuation(x)))])
    #     for sub in subject
    # ])
    # END PROBLEM 2
...
def accuracy(typed, source):
    ...
    if i == 0:
        if len(typed_words) == 0 and len(source_words) == 0:
            return 100.0
        return 0.0
    return 100 * match_cnt / len(typed_words)
    # END PROBLEM 3
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    # BEGIN PROBLEM 5
    "*** YOUR CODE HERE ***"
    diff, ret = -1, typed_word
    for w in word_list:
        if typed_word == w:
            return w
        cur_diff = diff_function(typed_word, w, limit)
        if diff == -1 or cur_diff < diff:
            diff = cur_diff
            ret = w
    return ret if diff <= limit else typed_word
    # END PROBLEM 5
...
def furry_fixes(typed, source, limit):
    ...
    def helper(cnt, a, b):
        ...
        if len(b) == 0:
            return cnt + len(a)
        cnt += 1 if a[0] != b[0] else 0
        return helper(cnt, a[1:], b[1:])
...
def minimum_mewtations(typed, source, limit):
    ...
    while (
        len(typed) > 0 and len(source) > 0 and typed[0] == source[0]
    ...
    removed = minimum_mewtations(typed[1:], source, limit - 1)
    substitute = minimum_mewtations(typed[1:], source[1:], limit - 1)
    # BEGIN
    return 1 + min(add, removed, substitute)
    # END
...
def final_diff(typed, source, limit):
    ...
    # Take into account which additions and deletions are more likely than others. For example, it's much more likely
    # that you'll accidentally leave out a letter if it appears twice in a row.
...
def fastest_words(match_object):
    ...
    for word_idx in word_indices:
        ...
        #     cur_cost = get_time(match_object, player_idx, word_idx)
        #     if cost == -1 or cur_cost < cost:
        #         cost = cur_cost
        #         ret_idx = player_idx
        # ret[ret_idx] = ret[ret_idx] + [get_word(match_object, word_idx)]
        ret_idx, _ = min(
            (
                (player_idx, get_time(match_object, player_idx, word_idx))
                for player_idx in player_indices
            ),
            key=lambda x: x[1],
        )
        ret[ret_idx].append(get_word(match_object, word_idx))
    return ret
