...
def insert_items(s, before, after):
    ...
    "*** YOUR CODE HERE ***"
    inserted = 0
    for ori_idx in range(len(s)):
        idx = ori_idx + inserted
        if s[idx] == before:
...
def balanced(m):
    ...
    if is_planet(m):
        return True
    if is_mobile(m):
        arml, armr = left(m), right(m)
        endl, endr = end(arml), end(armr)
        return (
            total_mass(endl) * length(arml) == total_mass(endr) * length(armr)
            and balanced(endl)
            and balanced(endr)
        )
    return False
