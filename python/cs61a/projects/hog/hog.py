...
def play(strategy0, strategy1, update, score0=0, score1=0, dice=six_sided, goal=GOAL):
    ...
    def inner_update(strategy, player_score, opponent_score):
        return update(
            strategy(player_score, opponent_score), player_score, opponent_score, dice
        )

    while score0 < goal and score1 < goal:
        if who == 0:
            score0 = inner_update(strategy0, score0, score1)
        else:
            score1 = inner_update(strategy1, score1, score0)
        who = 1 - who
    # END PROBLEM 5
...
def max_scoring_num_rolls(dice=six_sided, samples_count=1000):
    ...
    score, roll, ret_roll = 0, 1, 0
    while roll <= 10:
        cur = averaged_dice(roll, dice)
        if cur > score:
            score = cur
            ret_roll = roll
        roll += 1
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    """
    if (score > opponent_score) and (GOAL - score <= 10):
        roll_score, roll, cur_roll = (
            sus_update(0, score, opponent_score) - score,
            0,
            1,
        )
        while cur_roll <= 2:
            cur_score = make_averaged(roll_dice, 50)(cur_roll)
            if cur_score > roll_score:
                roll = cur_roll
                roll_score = cur_score
            cur_roll += 1
        return roll
    # BEGIN PROBLEM 12
    if sus_update(0, score, opponent_score) - score >= make_averaged(roll_dice, 50)(6):
        return 0
